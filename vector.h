...
public:
    ...
    iterator erase(const_iterator first, const_iterator last) {
        const_iterator finish = end();
        auto elems_after_last = (iterator) last;
        auto space = (iterator) first;
        size_t del_count = 0;
        while (first < last) {
            ++first;
            ++del_count;
...
private:
    ...
    void change_array_capacity(size_t nw_capacity) {
        auto *tmp_info = (size_t *) copy_array(nw_capacity);
        if (tmp_info == nullptr) {
            throw std::runtime_error("Wrong allocate");
        }
        auto *arr = (value_type *) (tmp_info + 3);
        free_data();
        info = tmp_info;
        array = arr;
    }
